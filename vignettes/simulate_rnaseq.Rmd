...
The airway dataset comes with few observed variables which affect gene
expression. We'll consider these "surrogate variables". That is, we'll
add signal to the data and trying to account for these surrogate variables.
```{r}
...
Does the sva-DESeq2 pipeline do any better? First, we'll fit DESeq2.
```{r, message=FALSE}
